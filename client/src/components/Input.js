...
function Input({ props }) {
    const { key, placeHolder, type, setInput, input, error, errorKey } = props;
    ...
    return (
            ...
            <div className="relative container mx-auto">
                <input
                    ...
                    type={(type === 'text' || isPasswordVisible) ? 'text' : 'password'}
                    className={`text-md block px-3 py-2 rounded-md w-full 
                        bg-white border-2 ${key === errorKey ? 'border-red-500 text-red-600' : 'border-gray-300'} 
